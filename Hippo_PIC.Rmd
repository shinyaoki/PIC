...
```{r totalReads, fig.width=8,fig.height=5}
bychr <- mat %>% select(-(1:3)) %>%
  gather("sample","count",-chr) %>%
  group_by(chr,sample) %>% summarise(total=sum(count)) %>% ungroup
ggplot(bychr,aes(reorder(sample,plyr::desc(sample)),total/1e6,fill=chr)) +
...
```{r biotype,fig.width=8,fig.height=6}
...
### DEG のみの normalized count データを作成 ## 不要
```{r deg_normalized, fig.width=8, fig.height=5}
deg_cnt <- dplyr::left_join(fc, cnt, by = "ens_gene")[, c(2, length(fc)+3:length(cnt)-1)]
deg_genes <- deg_cnt[, 1] %>% t
deg_cnt <- deg_cnt[, 2:length(deg_cnt)]
rownames(deg_cnt) <- deg_genes
deg_cnt2 <- transform(t(deg_cnt), group=def$group)

heatmap(t(deg_cnt), scale = "column")

readr::write_csv(deg_cnt, paste(outDir, "deg_", projName, ".csv", sep=""))
write.csv(deg_cnt, paste(outDir, "deg_", projName, ".csv", sep=""))
```
...
### Regularized log を tsne で tSNE
```{r tsne, fig.width=6, fig.height=2}
lX3.tsne <- tsne(t(lX3[rank(-apply(lX3,1,var)) <= ntop,]), max_iter=1000, perplexity = 3000)
df4 <- cbind(data.frame(lX3.tsne), df3)[, c(5,6,1,2)]
ggpoints(df4,modifyList(aes(X1,X2),myaes))
```

### Regularized log を umap で UMAP
```{r umap, fig.width=6, fig.height=2}
lX3.umap <- umap(t(lX3[rank(-apply(lX3,1,var)) <= ntop,]), pca = 10)
df5 <- cbind(data.frame(lX3.umap$layout), df3)[, c(5,6,1,2)]
if(exists("df5")) readr::write_csv(df5, paste(outDir, "UMAP_", projName, ".csv", sep=""))
ggpoints(df5,modifyList(aes(X1,X2),myaes))
```

### DEG の regularized log の k-means
```{r k-means, fig.width=6, fig.height=5}
lX4 <- transform(lX3, ens_gene=rownames(lX3))
lX4 <- dplyr::left_join(fc, lX4, by = "ens_gene")[, c(1, length(fc)+2:length(lX4)-1)]

K <- 30
km4 <- array(0, dim = c(K, dim(lX4)))

for (k in 1:K) {
km <- kmeans(lX4[, 2:length(lX4)] %>% as.matrix(), k, iter.max = 100)
  km2 <- transform(lX4, cluster=km$cluster)
  km3 <- km2[order(km2$cluster),][, 1:length(lX4)+1] %>% as.matrix()
  km4[k,,] <- km3
  rownames(km3) <- sub("1", "=", sub("0", "  =", km3[, ncol(km3)] %% 2))
  heatmap(km3[, 1:ncol(km3)-1], Colv = NA, Rowv = NA, scale = "row", main = k)
}
```
